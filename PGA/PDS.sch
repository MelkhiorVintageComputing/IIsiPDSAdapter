EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 3 5
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text GLabel 4750 5200 0    50   Input ~ 0
CACHE
Text GLabel 4750 5700 0    50   Input ~ 0
~CBACK
Text GLabel 4750 1600 0    50   Input ~ 0
~CIOUT
Text GLabel 4750 3600 0    50   Input ~ 0
A29
Text GLabel 5750 1500 2    50   Input ~ 0
A28
Text GLabel 4750 5800 0    50   Input ~ 0
~DSACK0
Text GLabel 5750 3800 2    50   Input ~ 0
~AS
Text GLabel 4750 6000 0    50   Input ~ 0
~BG
Text GLabel 4750 1800 0    50   Input ~ 0
~DSACK1
Text GLabel 5750 3600 2    50   Input ~ 0
~CBREQ
Text GLabel 4750 1700 0    50   Input ~ 0
~STERM
Text GLabel 5750 5400 2    50   Input ~ 0
A30
Text GLabel 4750 4800 0    50   Input ~ 0
CPUCLK
Text GLabel 5750 3300 2    50   Input ~ 0
~IRQ1
Text GLabel 5750 4100 2    50   Input ~ 0
~HALT
Text GLabel 4750 2100 0    50   Input ~ 0
FC2
Text GLabel 5750 4000 2    50   Input ~ 0
FC0
Text GLabel 5750 3500 2    50   Input ~ 0
RMC
Text GLabel 5750 5300 2    50   Input ~ 0
D31
Text GLabel 5750 5200 2    50   Input ~ 0
D28
Text GLabel 5750 5100 2    50   Input ~ 0
D25
Text GLabel 4750 7100 0    50   Input ~ 0
D22
Text GLabel 4750 7000 0    50   Input ~ 0
D19
Text GLabel 4750 2900 0    50   Input ~ 0
D16
Text GLabel 4750 2800 0    50   Input ~ 0
D13
Text GLabel 4750 2700 0    50   Input ~ 0
D10
Text GLabel 4750 2000 0    50   Input ~ 0
~BGACK
Text GLabel 5750 2500 2    50   Input ~ 0
A1
Text GLabel 4750 3700 0    50   Input ~ 0
A26
Text GLabel 4750 3800 0    50   Input ~ 0
A23
Text GLabel 5750 1800 2    50   Input ~ 0
A20
Text GLabel 5750 3400 2    50   Input ~ 0
IPL0
Text GLabel 4750 2400 0    50   Input ~ 0
D2
Text GLabel 5750 4200 2    50   Input ~ 0
D1
Text GLabel 5750 2400 2    50   Input ~ 0
A4
Text GLabel 5750 6300 2    50   Input ~ 0
A6
Text GLabel 5750 6100 2    50   Input ~ 0
A11
Text GLabel 5750 2200 2    50   Input ~ 0
A9
Text GLabel 5750 5900 2    50   Input ~ 0
A16
Text GLabel 4750 4000 0    50   Input ~ 0
A18
$Comp
L power:+12V #PWR0118
U 1 1 631F2920
P 5750 6900
F 0 "#PWR0118" H 5750 6750 50  0001 C CNN
F 1 "+12V" V 5765 7028 50  0000 L CNN
F 2 "" H 5750 6900 50  0001 C CNN
F 3 "" H 5750 6900 50  0001 C CNN
	1    5750 6900
	0    1    1    0   
$EndComp
Text GLabel 5750 3700 2    50   Input ~ 0
~RW
Text GLabel 4750 1900 0    50   Input ~ 0
SIZ1
Text GLabel 5750 1300 2    50   Input ~ 0
D30
Text GLabel 5750 1200 2    50   Input ~ 0
D27
Text GLabel 4750 3200 0    50   Input ~ 0
D24
Text GLabel 4750 3100 0    50   Input ~ 0
D21
Text GLabel 4750 3000 0    50   Input ~ 0
D18
Text GLabel 5750 4700 2    50   Input ~ 0
D15
Text GLabel 5750 4600 2    50   Input ~ 0
D12
Text GLabel 5750 4500 2    50   Input ~ 0
D9
Text GLabel 5750 3900 2    50   Input ~ 0
~BR
Text GLabel 4750 3500 0    50   Input ~ 0
A31
Text GLabel 5750 1600 2    50   Input ~ 0
A25
Text GLabel 5750 5700 2    50   Input ~ 0
A22
Text GLabel 4750 1500 0    50   Input ~ 0
IPL2
Text GLabel 4750 6400 0    50   Input ~ 0
D3
Text GLabel 4750 2500 0    50   Input ~ 0
D5
Text GLabel 4750 2300 0    50   Input ~ 0
D0
Text GLabel 4750 4600 0    50   Input ~ 0
A2
Text GLabel 5750 2100 2    50   Input ~ 0
A12
Text GLabel 4750 4200 0    50   Input ~ 0
A13
Text GLabel 5750 6200 2    50   Input ~ 0
A8
Text GLabel 4750 4100 0    50   Input ~ 0
A15
Text GLabel 5750 1900 2    50   Input ~ 0
A17
Text GLabel 4750 5600 0    50   Input ~ 0
~DS
Text GLabel 4750 6200 0    50   Input ~ 0
~BERR
Text GLabel 4750 5900 0    50   Input ~ 0
SIZ0
Text GLabel 4750 6100 0    50   Input ~ 0
FC1
Text GLabel 4750 2200 0    50   Input ~ 0
~RESET
Text GLabel 4750 3400 0    50   Input ~ 0
D29
Text GLabel 4750 3300 0    50   Input ~ 0
D26
Text GLabel 5750 5000 2    50   Input ~ 0
D23
Text GLabel 5750 4900 2    50   Input ~ 0
D20
Text GLabel 5750 4800 2    50   Input ~ 0
D17
Text GLabel 4750 6800 0    50   Input ~ 0
D14
Text GLabel 4750 6700 0    50   Input ~ 0
D11
Text GLabel 4750 2600 0    50   Input ~ 0
D8
Text GLabel 5750 6500 2    50   Input ~ 0
A0
Text GLabel 5750 5500 2    50   Input ~ 0
A27
Text GLabel 5750 5600 2    50   Input ~ 0
A24
Text GLabel 4750 3900 0    50   Input ~ 0
A21
Text GLabel 4750 5500 0    50   Input ~ 0
IPL1
Text GLabel 5750 4300 2    50   Input ~ 0
D4
Text GLabel 4750 6500 0    50   Input ~ 0
D6
Text GLabel 5750 4400 2    50   Input ~ 0
D7
Text GLabel 5750 6400 2    50   Input ~ 0
A3
Text GLabel 4750 4500 0    50   Input ~ 0
A5
Text GLabel 4750 4400 0    50   Input ~ 0
A7
Text GLabel 4750 4300 0    50   Input ~ 0
A10
Text GLabel 5750 6000 2    50   Input ~ 0
A14
Text GLabel 5750 5800 2    50   Input ~ 0
A19
$Comp
L power:-5V #PWR0119
U 1 1 631F5730
P 5750 2900
F 0 "#PWR0119" H 5750 3000 50  0001 C CNN
F 1 "-5V" V 5765 3028 50  0000 L CNN
F 2 "" H 5750 2900 50  0001 C CNN
F 3 "" H 5750 2900 50  0001 C CNN
	1    5750 2900
	0    1    1    0   
$EndComp
Text GLabel 4750 5400 0    50   Input ~ 0
~IRQ2
Text GLabel 4750 1400 0    50   Input ~ 0
~IRQ3
Text GLabel 5750 3200 2    50   Input Italic 0
~TM0A
Text GLabel 4750 5300 0    50   Input Italic 0
~TM1A
Text GLabel 4750 1300 0    50   Input Italic 0
~BUSLOCk
Text GLabel 5750 6700 2    50   Input ~ 0
C16M
Text GLabel 4750 1200 0    50   Input ~ 0
~FPU
Text GLabel 5750 3100 2    50   Input Italic 0
~NUBUS
Text GLabel 5750 2700 2    50   Input Italic 0
ECLK
Text GLabel 6050 2700 2    50   Input Italic 0
VIACLK
Text GLabel 5750 3000 2    50   Input Italic 0
~PFW
Text GLabel 4750 5100 0    50   Input Italic 0
C40M
Text GLabel 4750 1100 0    50   Input Italic 0
~RBV
$Comp
L power:-12V #PWR0120
U 1 1 64456710
P 4750 5000
F 0 "#PWR0120" H 4750 5100 50  0001 C CNN
F 1 "-12V" V 4765 5128 50  0000 L CNN
F 2 "" H 4750 5000 50  0001 C CNN
F 3 "" H 4750 5000 50  0001 C CNN
	1    4750 5000
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0121
U 1 1 64EC2A93
P 4750 4900
F 0 "#PWR0121" H 4750 4650 50  0001 C CNN
F 1 "GND" V 4755 4772 50  0000 R CNN
F 2 "" H 4750 4900 50  0001 C CNN
F 3 "" H 4750 4900 50  0001 C CNN
	1    4750 4900
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0122
U 1 1 64EC2A94
P 4750 6600
F 0 "#PWR0122" H 4750 6350 50  0001 C CNN
F 1 "GND" V 4755 6472 50  0000 R CNN
F 2 "" H 4750 6600 50  0001 C CNN
F 3 "" H 4750 6600 50  0001 C CNN
	1    4750 6600
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0123
U 1 1 64478CBA
P 5750 1100
F 0 "#PWR0123" H 5750 850 50  0001 C CNN
F 1 "GND" V 5755 972 50  0000 R CNN
F 2 "" H 5750 1100 50  0001 C CNN
F 3 "" H 5750 1100 50  0001 C CNN
	1    5750 1100
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0124
U 1 1 6447AF14
P 5750 1700
F 0 "#PWR0124" H 5750 1450 50  0001 C CNN
F 1 "GND" V 5755 1572 50  0000 R CNN
F 2 "" H 5750 1700 50  0001 C CNN
F 3 "" H 5750 1700 50  0001 C CNN
	1    5750 1700
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0125
U 1 1 64EC2A97
P 5750 2300
F 0 "#PWR0125" H 5750 2050 50  0001 C CNN
F 1 "GND" V 5755 2172 50  0000 R CNN
F 2 "" H 5750 2300 50  0001 C CNN
F 3 "" H 5750 2300 50  0001 C CNN
	1    5750 2300
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0126
U 1 1 64EC2A98
P 5750 2800
F 0 "#PWR0126" H 5750 2550 50  0001 C CNN
F 1 "GND" V 5755 2672 50  0000 R CNN
F 2 "" H 5750 2800 50  0001 C CNN
F 3 "" H 5750 2800 50  0001 C CNN
	1    5750 2800
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0127
U 1 1 64EC2A99
P 5750 6800
F 0 "#PWR0127" H 5750 6550 50  0001 C CNN
F 1 "GND" V 5755 6672 50  0000 R CNN
F 2 "" H 5750 6800 50  0001 C CNN
F 3 "" H 5750 6800 50  0001 C CNN
	1    5750 6800
	0    -1   -1   0   
$EndComp
$Comp
L power:+5V #PWR0128
U 1 1 64EC2A9A
P 4750 6300
F 0 "#PWR0128" H 4750 6150 50  0001 C CNN
F 1 "+5V" V 4765 6428 50  0000 L CNN
F 2 "" H 4750 6300 50  0001 C CNN
F 3 "" H 4750 6300 50  0001 C CNN
	1    4750 6300
	0    -1   -1   0   
$EndComp
$Comp
L power:+5V #PWR0129
U 1 1 64484CC0
P 4750 6900
F 0 "#PWR0129" H 4750 6750 50  0001 C CNN
F 1 "+5V" V 4765 7028 50  0000 L CNN
F 2 "" H 4750 6900 50  0001 C CNN
F 3 "" H 4750 6900 50  0001 C CNN
	1    4750 6900
	0    -1   -1   0   
$EndComp
$Comp
L power:+5V #PWR0130
U 1 1 64EC2A9C
P 5750 2000
F 0 "#PWR0130" H 5750 1850 50  0001 C CNN
F 1 "+5V" V 5765 2128 50  0000 L CNN
F 2 "" H 5750 2000 50  0001 C CNN
F 3 "" H 5750 2000 50  0001 C CNN
	1    5750 2000
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0131
U 1 1 6448744C
P 5750 1400
F 0 "#PWR0131" H 5750 1250 50  0001 C CNN
F 1 "+5V" V 5765 1528 50  0000 L CNN
F 2 "" H 5750 1400 50  0001 C CNN
F 3 "" H 5750 1400 50  0001 C CNN
	1    5750 1400
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0132
U 1 1 64487EE0
P 5750 2600
F 0 "#PWR0132" H 5750 2450 50  0001 C CNN
F 1 "+5V" V 5765 2728 50  0000 L CNN
F 2 "" H 5750 2600 50  0001 C CNN
F 3 "" H 5750 2600 50  0001 C CNN
	1    5750 2600
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0133
U 1 1 64488E65
P 5750 6600
F 0 "#PWR0133" H 5750 6450 50  0001 C CNN
F 1 "+5V" V 5765 6728 50  0000 L CNN
F 2 "" H 5750 6600 50  0001 C CNN
F 3 "" H 5750 6600 50  0001 C CNN
	1    5750 6600
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0134
U 1 1 6448A9CE
P 4750 4700
F 0 "#PWR0134" H 4750 4550 50  0001 C CNN
F 1 "+5V" V 4765 4827 50  0000 L CNN
F 2 "" H 4750 4700 50  0001 C CNN
F 3 "" H 4750 4700 50  0001 C CNN
	1    4750 4700
	0    -1   -1   0   
$EndComp
$Comp
L 5535098-5:5535098-5 J3
U 1 1 64EC58FD
P 4750 1100
F 0 "J3" H 5250 1365 50  0000 C CNN
F 1 "5535098-5" H 5250 1274 50  0000 C CNN
F 2 "For_SeeedStudio:5535098-5" H 5600 1200 50  0001 L CNN
F 3 "https://www.te.com/commerce/DocumentDelivery/DDEController?Action=srchrtrv&DocNm=5535098&DocType=Customer+Drawing&DocLang=English" H 5600 1100 50  0001 L CNN
F 4 "120 Position Din Connector Receptacle, Female Sockets Gold Through Hole" H 5600 1000 50  0001 L CNN "Description"
F 5 "11.51" H 5600 900 50  0001 L CNN "Height"
F 6 "TE Connectivity" H 5600 800 50  0001 L CNN "Manufacturer_Name"
F 7 "5535098-5" H 5600 700 50  0001 L CNN "Manufacturer_Part_Number"
F 8 "571-5535098-5" H 5600 600 50  0001 L CNN "Mouser Part Number"
F 9 "https://www.mouser.co.uk/ProductDetail/TE-Connectivity/5535098-5?qs=H1z6Bf8FAOuC4CcPF8q3gg%3D%3D" H 5600 500 50  0001 L CNN "Mouser Price/Stock"
F 10 "5535098-5" H 5600 400 50  0001 L CNN "Arrow Part Number"
F 11 "https://www.arrow.com/en/products/5535098-5/te-connectivity" H 5600 300 50  0001 L CNN "Arrow Price/Stock"
F 12 "DNP" H 4750 1100 50  0001 C CNN "DNP"
	1    4750 1100
	1    0    0    -1  
$EndComp
Text Notes 4275 5225 0    50   ~ 0
GND
Text Notes 4250 5125 0    50   ~ 0
RSVD
Text Notes 4300 1125 0    50   ~ 0
RSVD
Text Notes 4300 1225 0    50   ~ 0
RSVD
$EndSCHEMATC
